...
class TikaApp extends GestureEventListeners(PolymerElement) {
  static get template() {
    return html`
      ...
      <section class="layout vertical center">
        <div class="container">
        <h1>[[title]]</h1>
          <p><span class="block">[[title]]</span> provides a text-extraction
          service using <span class="block">Apache Tika<sup>TM</sup></span>.
          You can extract the text by dropping your <span class="block">doc</span>,
          <span class="block">docx</span>, <span class="block">pdf</span>, and
          <span class="block">images</span> into the section below.
          </p>
      ...
      <paper-fab id="submit"
        icon="play-arrow"
        disabled="[[_readyToProcess(files, processing)]]"
        on-click="_onStartJobs"></paper-fab>
  ...
  async _runJob(jobId, output='plain') {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this._showErrorMsg({ message: `ERROR${response.status}: ${response.statusText}` });
      }
    } catch (error) {
  ...
  _renderError(error, size) {
    ...
    else return this._invalidSize(size) ? `Max file size allowed is ${this._returnFileSize(this.maxSize)}!`: '';
  }
}
